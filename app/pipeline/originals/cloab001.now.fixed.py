# -*- coding: utf-8 -*-
# ============================================================
# 実行環境共通ブートストラップ（Colab / Cloud Run 互換）
# - --workdir で入出力ディレクトリを切り替え（既存版 cloab001.py と同じI/O）
# - anthropic が未インストールなら自動インストール
...
# google.colab.userdata が無い環境でも動くようにする
try:
    ...
    userdata = _UserDataFallback()
"""１－１.Claude_Ver

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1tD7V6hBTFBjZzeYlEbdYosN95ld4myCm
"""
...
# -*- coding: utf-8 -*-
"""１ー１．（新）データ加工 create_output.json（共通）

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1wXiFI-HMNPOMbxB8QYWADbAAfyLlDmLW
"""
...
api_key = userdata.get('ANTHROPIC_API_KEY')
if not api_key:
    raise RuntimeError("Colab の [⋮]→[ユーザーシークレット] に 'ANTHROPIC_API_KEY' を登録してください。")
...
# 1. 1_78.json の出力
output_json_path = Path("1_78.json")
...
output_csv_path = Path("check_1_78.csv")
with output_csv_path.open("w", encoding="cp932", newline="") as f:
    writer = csv.writer(f)
    # ヘッダー行
...
response_pl_112_120 = client.messages.create(
    model=MODEL,
    system=system_prompt_pl_112_120,
    messages=[
...
raw_text_pl_112_120 = ""
for item in response_pl_112_120.content:
    if item.type == "text":
        raw_text_pl_112_120 += item.text

if not raw_text_pl_112_120.strip():
    print("---- デバッグ：response_pl_112_120.content ----")
    print(response_pl_112_120.content)
...
# ============================================================
# 行112（売上高）をPDF記載値（data.json）で上書き【最優先】
# PLに複数の売上列がある場合、一番右（最終）の売上合計を採用する
# ルール：「売上高」勘定科目 かつ 分類が「売上高」のものを最優先。
#         複数ある場合は最後に出現したものを売上合計と見なす。
# ============================================================

def _override_row112_from_pl(rows_pl_112_120, source_data):
    """
    data.json の PL 配列から売上高合計を取得して行112を上書きする。
    複数列ある場合は「一番右（最後に出現する売上合計行）」を採用。
    """
    ...
    pl_items = source_data.get("PL", [])

    # ステップ1: 分類が「売上高」かつ勘定科目が「売上高」の行を収集（複数期対応）
    # 複数ある場合は最後のものが一番右の合計列
    candidates_exact = []
    for item in pl_items:
        name = str(item.get("勘定科目", "")).strip()
        bunrui = str(item.get("分類", "")).strip()
        if name == "売上高" and bunrui == "売上高":
            candidates_exact.append(item)

    # ステップ2: 分類が「売上高」の合計行（合計っぽい名称）を収集
    candidates_total = []
    if not candidates_exact:
        total_keywords = ["売上高合計", "売上合計", "純売上高", "正味売上高", "事業収益合計", "完成工事高合計"]
        for item in pl_items:
            name = str(item.get("勘定科目", "")).strip()
            bunrui = str(item.get("分類", "")).strip()
            if name in total_keywords and bunrui in ("売上高", ""):
                candidates_total.append(item)

    # ステップ3: 採用する候補を決定（最後に出現したものが一番右の合計列）
    chosen = None
    method_note = ""
    if candidates_exact:
        chosen = candidates_exact[-1]  # 複数ある場合は最後＝一番右
        method_note = f"PDF記載値を優先採用（勘定科目=売上高, 分類=売上高）" + (
            f"（{len(candidates_exact)}件中最後の値を採用）" if len(candidates_exact) > 1 else ""
        )
    elif candidates_total:
        chosen = candidates_total[-1]
        method_note = f"PDF記載の売上合計行を採用: {chosen.get('勘定科目','')}"

    if chosen is None:
        # 候補なし → LLMの結果をそのまま使う
        print("⚠ 行112: data.json に売上高合計行が見つからなかったため、LLM出力をそのまま使用します。")
        return

    # ステップ4: 金額取得（dict形式と直値形式の両方に対応）
    def _get_val(item, period):
        v = item.get(period, {})
        if isinstance(v, dict):
            return _safe_int(v.get("金額", 0))
        return _safe_int(v)

    now_val   = _get_val(chosen, "今期")
    prev_val  = _get_val(chosen, "前期")
    prev2_val = _get_val(chosen, "前々期")

    # ステップ5: rows_pl_112_120 の行112を上書き
    for row in rows_pl_112_120:
        if row["行番号"] == 112:
            old_vals = (row["今期"], row["前期"], row["前々期"])
            row["今期"]    = now_val
            row["前期"]    = prev_val
            row["前々期"]  = prev2_val
            row["集計方法"] = method_note
            print(f"✅ 行112 売上高 上書き完了: 前々期 {old_vals[2]} → {prev2_val}, 前期 {old_vals[1]} → {prev_val}, 今期 {old_vals[0]} → {now_val}")
            break

_override_row112_from_pl(rows_pl_112_120, source_data)
...
response_pl = client.messages.create(
    model=MODEL,
    system=system_prompt_pl,
    messages=[
...
raw_text_pl = ""
for item in response_pl.content:
    if item.type == "text":
        raw_text_pl += item.text

if not raw_text_pl.strip():
    print("---- デバッグ：response_pl.content ----")
    print(response_pl.content)
...
# ============================================================
# 147: 営業外支出その他（148行目 - 146行目）
# ※ 148行目（営業外支出合計）が確定した後に計算するため、148の計算後に再設定
# ============================================================

# 147は148確定後に計算するため、ここでは仮置き（後で上書き）
set_vals(147, [0, 0, 0])
row_dict[147]["集計方法"] = "148行目 - 146行目"
...
# 147: 営業外支出その他 = 148行目 - 146行目
vals148 = get_vals(148)
vals146 = get_vals(146)
set_vals(147, [vals148[j] - vals146[j] for j in range(3)])
row_dict[147]["集計方法"] = "148行目 - 146行目"
